...
tokens = ('ID', 'NUM', 'COLON', 'OR', 'EQ', 'GT', 'LT', 'GE', 'LE', 'NE',
          'AND', 'NOT', 'LPAREN', "RPAREN", "ORR")
...
def apply_or(conditions):
  if len(conditions) == 1:
    return conditions[0]
  return or_(apply_or(conditions[:len(conditions) // 2]),
...
def parse(query: str) -> list[tuple[CourseSection, list[RoomSchedule]]]:
  ...
  with Session(engine) as session:
    ...
    section_ids = set([
        course_section.id for course_section in session.query(
            CourseSection).join(RoomSchedule).filter(parsed_query).all()
    ])
    ...
    for section in sections:
      section_schedules.append((section, session.query(RoomSchedule).filter(
          RoomSchedule.course_section_id == section.id).all()))
